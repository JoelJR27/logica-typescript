...
function contaPalavras(palavra: string) {
  const regEx = /[!"#$%&'()*+,\-./:;<=>?@[\\\]^_`{|}~]/g;
  let palavraSemPontuaçao: string = "";
  for (let index = 0; index < palavra.length; index++) {
    if (!palavra[index].match(regEx)) palavraSemPontuaçao += palavra[index];
  }
  return palavraSemPontuaçao.split(" ").filter((caractere: string) => caractere)
    .length;
}
