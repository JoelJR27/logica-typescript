...
function criarTabuleiro(tamanhoDoTabuleiro: number) {
  const matrizNxN = [];
  let index = 0;
  while (index < tamanhoDoTabuleiro) {
    let linha: string[] = criarLinhasDoTabuleiro(tamanhoDoTabuleiro);
...
function criarLinhasDoTabuleiro(tamanhoDaLinha: number) {
  const linhaDaMatriz: string[] = [];
  let index = 0;
